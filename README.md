...
| 「問い合わせ」 | サポーターが多い号車を検索 | テイカー |
| 「座席リクエスト」 | 同じ号車のサポーターに依頼を送信 | テイカー |
| 「マッチ確認」 | マッチング結果を確認 | テイカー |
| 「キャンセル」 | 操作を中断 | 共通 |
...
```
「マッチ確認」と送信
  → マッチング済みの場合：列車番号・号車・座席位置を表示
  → 未マッチの場合：待機中のメッセージを表示
```
...
### プッシュ通知の /notify 呼び出し（バックエンドチームへの依頼）
...
| `/match/candidate` 処理後 | `thanks` | サポーター |
